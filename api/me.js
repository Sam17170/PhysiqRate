...
export default async function handler(req) {
  ...
  if (!user.email) return new Response(JSON.stringify({ error: "Invalid token" }), { status: 401 });

  // Get pro status from users table
  const dbRes = await fetch(`${SUPABASE_URL}/rest/v1/users?email=eq.${encodeURIComponent(user.email)}&select=is_pro,stripe_session_id`, {
    headers: { "apikey": SUPABASE_ANON, "Authorization": `Bearer ${SUPABASE_ANON}` }
  });
  const rows = await dbRes.json();
  const userData = rows[0] || { is_pro: false };
